    ...
    if (isLoading && products.length === 0) {
        return (
            <div className="flex h-full flex-col items-center justify-center gap-4">
                ...
                {/* Fallback if stuck */}
                <button onClick={() => window.location.reload()} className="text-sm text-rose-500 underline mt-4">
                    Reload App
                </button>
            </div>
        );
        ...
        return (
            <div className="flex h-full flex-col items-center justify-center gap-4 p-6">
                <div className="bg-gray-50 p-6 rounded-2xl flex flex-col items-center text-center max-w-sm">
                    ...
                    <p className="text-gray-500 text-sm mb-4">The menu appears to be empty. Please check your inventory or sync settings.</p>
                    <button
                        onClick={() => fetchMenu({ tenantId, outletId })}
